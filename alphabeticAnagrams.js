...
function calcPermutations(freq) {
  let totalLetters = Object.values(freq).reduce((sum, val) => sum + val, 0);
  let permutations = factorial(totalLetters);

  Object.values(freq).forEach((val) => {
    permutations /= factorial(val);
  });
...
function listPosition(word) {
  ...
  let freq = {};
  letters.forEach((letter) => (freq[letter] = (freq[letter] || 0) + 1));
  ...
  for (let i = 0; i < len; i++) {
    for (
      ...
      if (freq[ch]) {
        freq[ch]--;
        rank += calcPermutations(freq);
        freq[ch]++;
      }
    }
    freq[letters[i]]--;
    if (freq[letters[i]] === 0) delete freq[letters[i]];
  }
